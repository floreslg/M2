...
#define VAL_D 800
  ...
  INTEGER                      :: i, j, ir, t0, t1, iteration=0, N_threads
  ...
  Jaco : DO
     ...
     !$OMP DO  PRIVATE(j) SCHEDULE(RUNTIME)
     DO i=1,n
        x_courant(i) = 0.0
        !
        DO j=1,i-1
  ...
  PRINT '(//, 3x, "Taille du systeme   : ", i5, /       &
    ...
    INQUIRE(file="report_jacob.csv", exist=exist)
    
    IF (exist) THEN
        OPEN(1, file="report_jacob.csv", &
                status="old", &
                position="append", &
                action="write")
    ELSE
        OPEN(1, file="report_jacob.csv", status="new", action="write")
        WRITE(1, '(*(A10,:,","))') "taille", &
                "diag", &
                "iters", &
                "norme", &
