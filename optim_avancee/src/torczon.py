...
class Torczon():
    ...
    def _expand(self, x):
        x_exp = (1+self._gamma)*self.x_best - self._gamma*x
        return (x_exp, self.loss_func(x_exp))
    ...
    def optimize(self):
        ...
        for iter in tqdm(range(self.max_iter)):
            ...
            if True in [loss_ref < loss_best for _, loss_ref in ref_evals]:
                ...
                # expansion
                with Pool() as p:
                    new_evals += p.map(self._expand, [x for x,_ in evals[1:]])
...
if __name__ == "__main__":
    ...
    optim.optimize()
    optim.plot_loss('testTo', outdir='../figures')